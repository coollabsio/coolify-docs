...
const githubOwner = 'coollabsio';
const githubRepo = 'coolify';
...
export function GithubInfo({ className }: { className?: string }) {
    ...
    async function loadGithubInfo() {
      try {
        ...
        if (typeof data.stargazers_count === 'number') {
          setStars(humanizeNumber(data.stargazers_count));
        }
      } catch (error) {
  ...
  return (
    <span
      className={cn('inline-flex items-center gap-1.5 text-current transition-colors', className)}
      title={stars ? `${githubOwner}/${githubRepo}: ${stars} stars` : `${githubOwner}/${githubRepo}`}
    >
      ...
      {stars ? <span className="text-xs font-bold">{stars}</span> : null}
    </span>
  );
}
