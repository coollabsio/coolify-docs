...
You can expose the Coolify dashboard, realtime updates, and terminal connections through a single tunnel route.

### 1. Create a route in Cloudflare Tunnel [!toc]

Follow [Step 5](/integrations/cloudflare/tunnels/single-resource#5-configure-tunnel-routes) and use the following mapping:

| Domain | Service | Purpose |
|---|---|---|
| `coolify.shadowarcanist.com` | `http://localhost:8000` | Coolify dashboard, realtime updates, and terminal |

Coolify routes `/app` and `/terminal/ws` internally, so you do not need separate tunnel routes for ports `6001` and `6002`. You also do not need to set `PUSHER_HOST`, `PUSHER_PORT`, `TERMINAL_HOST`, or `TERMINAL_PORT` for this setup.

### 2. Verify the Setup [!toc]
...
3. In your browser's network tab, confirm the realtime and terminal WebSocket connections use the Coolify dashboard hostname.


<Callout type="warn" title="HEADS UP!">

If you use a firewall, Cloudflare Tunnel only needs to reach the Coolify dashboard on port `8000`. Ports `6001` and `6002` do not need to be exposed publicly.

</Callout>

### Legacy multi-route setup [!toc]

Existing installations that route Coolify's WebSocket services separately can keep using the following configuration:
...
Keep the path-specific terminal route above the dashboard route so Cloudflare matches it first. If you use a separate realtime hostname, configure Coolify in `/data/coolify/source/.env`:
...
Explicit `PUSHER_HOST`, `PUSHER_PORT`, `TERMINAL_HOST`, and `TERMINAL_PORT` overrides remain supported. New installations should use the single-route setup above unless they need direct access to the realtime or terminal services.
