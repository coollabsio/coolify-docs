---
title: Domains
description: "Add custom domains to Coolify with FQDN format, multiple domain support, port mapping, path-based routing, per-domain noindex, wildcard domains, and custom DNS server validation."
---
...
Open your application's **General** settings. Under **Search Engine Indexing**, check the domains you want to exclude from indexing and the change is applied on save.


<Callout type="info" title="READONLY CONTAINER LABELS">

The **Search Engine Indexing** section only appears when at least one domain is configured and **Readonly container labels** is enabled for the application. If you manage your own container labels, add the `X-Robots-Tag` header manually instead.
...
Open the domain editor of the service and use the **Search Engine Indexing** section, which lists each configured domain with a checkbox.


<Callout type="info" title="GOOD TO KNOW">

- The `noindex` header overrides any `X-Robots-Tag` your application or service sets itself.
- Preview deployments are always served as `noindex`.
- Works with both the Traefik and Caddy proxies.
