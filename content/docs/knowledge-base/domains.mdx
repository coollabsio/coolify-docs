---
title: Domains
description: "Add custom domains to Coolify with FQDN format, multiple domain support, port mapping, path-based routing, search engine indexing control, wildcard domains, and custom DNS server validation."
---
...
Check the domains you want to exclude under **Search Engine Indexing**, in your application's **General** settings or in the domain editor of a service.

Since the header is applied by your reverse proxy, the details — including how to set it up manually if you manage your own container labels — live with each proxy: [Traefik](/knowledge-base/proxy/traefik/noindex) and [Caddy](/knowledge-base/proxy/caddy/noindex).
