...
This guide covers **PM2 cluster mode** (Node.js) and **`reusePort`** (Bun, Deno), with examples for the [Dockerfile](/applications/builds/dockerfile) and [Nixpacks](/applications/builds/nixpacks/overview) build packs.
...
Use this when your app is built with the [Dockerfile build pack](/applications/builds/dockerfile).
...
Use this when your app is built with the [Nixpacks build pack](/applications/builds/nixpacks/overview).

<Tabs id="nixpacks-pm2" items={['Environment variable', 'nixpacks.toml']}>
...
<Tabs id="js-multicore-build" items={['Dockerfile', 'nixpacks.toml']}>
