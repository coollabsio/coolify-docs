...
description: Select a Node.js major version in Nixpacks or pin a nixpkgs archive when an exact package set is required.
---
...
Keep **Build Variable** enabled, then redeploy and inspect the build log to confirm the resolved version.
...
The value is a complete commit SHA from the [NixOS/nixpkgs repository](https://github.com/NixOS/nixpkgs?utm_source=coolify.io). The example commit contains Node.js `22.13.1`.

Redeploy and inspect the Nixpacks build log to confirm the Node.js version selected from the pinned archive.

<Callout type="warning" title="An archive pin changes the complete package set">
...
Use a [Dockerfile](/applications/builds/dockerfile) with an exact base-image tag or digest when exact runtime reproducibility is a deployment requirement.

## Find an archive commit

To locate a nixpkgs commit for a required Node.js release:

1. Open the [NixOS/nixpkgs repository](https://github.com/NixOS/nixpkgs?utm_source=coolify.io).
2. Find the Node.js version definition, such as `pkgs/development/web/nodejs/v22.nix`.
3. Open the file history and find the commit that introduced the required version.
4. Confirm the `version` and source hash in that revision.
5. Copy the complete commit SHA into `nixpacks.toml`.
6. Deploy in a non-production environment and verify the resolved Node.js version in the build log.

For example, commit [`51ad838b03a05b1de6f9f2a0fffecee64a9788ee`](https://github.com/NixOS/nixpkgs/commit/51ad838b03a05b1de6f9f2a0fffecee64a9788ee) updates the Node.js 22 package from `22.12.0` to `22.13.1`.

Read the upstream [nixpkgs archive configuration](https://nixpacks.com/docs/configuration/file#nixpkgs-archive?utm_source=coolify.io) for the Nixpacks setting.
...
<Callout type="warning" title="Reference data only">
...
<Tabs id="node-version-hashes" items={['Node.js 20.x', 'Node.js 22.x', 'Node.js 24.x', 'Node.js 25.x']}>
...
Read the [Nixpacks Node provider](https://nixpacks.com/docs/providers/node?utm_source=coolify.io) for upstream version-selection behavior.
