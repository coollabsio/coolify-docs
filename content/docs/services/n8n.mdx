...
- **Database:** PostgreSQL + Redis
- **Use case:** Queue mode with n8n Assistant. Requires n8n 2.35+.
- **Components:**
  - n8n main container
  - n8n-worker and task-runners
  - PostgreSQL 16 and Redis
  - sandbox-certs, sandbox-api, sandbox-runner-1 (privileged Docker-in-Docker)
  - SearXNG (internal only)
- Set `N8N_INSTANCE_AI_MODEL_API_KEY` (or add a model key in n8n after login). Do not publish sandbox or SearXNG ports.
