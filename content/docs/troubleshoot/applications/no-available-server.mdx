...
If your deployed application or service shows a **"No Available Server"** error, this indicates that [Traefik](/core/networking/proxy/traefik/overview) (the reverse proxy) cannot find any (healthy) containers to route traffic to under the provided secured URL (`https`).
...
Verify that you entered the domain correctly in the application or service configuration and that its DNS records point to the correct IP address. See [Domains](/core/networking/domains) for the supported formats.
...
<Accordion title="Fix Failed Health Checks (Most Common)">
...
<Callout type="info">
   ...
   These are some common solutions to fix a health check. Adjust based on your specific application and health check command. Read more about [configuring health checks](/applications/configuration/health-checks).
...
<Accordion title="Fix Domain Configuration">
...
**Steps:**
Verify that your domain is correctly set up in both Coolify and your DNS provider. See [Domains](/core/networking/domains) for the complete setup.
...
<Accordion title="Fix Port Configuration">

**Symptoms:**

- Application / Service works via `http://IP:port` but not via domain (manual port mapping required)
- [Traefik](/core/networking/proxy/traefik/overview) can't reach the application
...
<Accordion title="Handle Deployment Downtime">
...
Ensure that rolling updates are correctly configured. See [Rolling updates](/applications/deployments/rolling-updates).

</Accordion>
<Accordion title="Update Traefik to Fix Docker API Version Issue">
...
<Accordion title="Check Traefik Configuration">
...
<Accordion title="Test Health Check from Inside Container">
