...
<Accordion title="Issue 1: Custom Docker Network Isolation">
...
<Tabs items={['Use Coolify Destinations (Recommended)', 'Manual Network Connection (Temporary)']}>
<Tab value="Use Coolify Destinations (Recommended)">

Let Coolify manage networks automatically by using [Destinations](/core/networking/destinations/overview) instead of custom networks:

1. Remove custom network definitions from your Docker Compose file
2. [Create the network destination](/core/networking/destinations/create) in Coolify under **Destinations**
3. [Connect the Docker Compose resource to its predefined destination](/core/networking/destinations/manage#connect-docker-compose-resources)
4. Redeploy
...
<Tab value="Manual Network Connection (Temporary)">
...
<Accordion title="Issue 2: Large Upload/Download Timeouts">
...
<Tabs items={['Increase Proxy Timeout', 'Implement Chunked Uploads', 'Use Background Processing']}>
<Tab value="Increase Proxy Timeout">

The configuration method depends on your proxy type:

<Tabs items={['Traefik', 'Caddy', 'Nginx']}>
<Tab value="Traefik">
...
<Tab value="Implement Chunked Uploads">
...
<Tab value="Use Background Processing">
